...
def get_next_post_id():
    numbers = [int(match.group(1)) for name in os.listdir(IMAGES_DIR) if (match := re.match(r"post_(\d+)\.jpg", name, re.I))]
    return f"post_{max(numbers) + 1 if numbers else 1:02d}"
...
def generate_hinglish_quote_and_caption():
    ...
    print(f"Today's Topic Focus: {bucket}")
    ...
    for model_name in MODELS_TO_TRY:
        for attempt in range(1, 4):
            try:
                ...
                if any(code in str(exc) for code in ("503", "UNAVAILABLE", "429")) and attempt < 3:
                    time.sleep(5)
                else:
                    break
    print("All content-generation models failed.")
    return None, None
...
def create_post():
    ...
    if not os.path.exists(BACKGROUND_PATH) or not os.path.exists(FONT_PATH):
        print("Error: missing branding background or font.")
        return False
    image = Image.open(BACKGROUND_PATH).convert("RGB")
    draw = ImageDraw.Draw(image)
    font_size = int(image.width * 0.055)
    try:
    ...
    y = image.height - (bbox[3] - bbox[1]) - int(image.height * 0.12)
    ...
    with open(os.path.join(CONTENT_DIR, f"{post_id}.txt"), "w", encoding="utf-8") as file:
