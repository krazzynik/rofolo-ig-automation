...
def main():
    ...
    if not item:
        print("No pending Instagram-only posts found.")
        return 0
    ...
    try:
        post_id = item["id"]
        filename, image_path = find_image(post_id)
        caption_path = os.path.join(CONTENT_DIR, f"{post_id}.txt")
        if not image_path or not os.path.exists(caption_path):
            raise InstagramOnlyError(f"Missing image or caption for {post_id}.")
        creation_id = item.get("instagram_creation_id")
        if not creation_id:
            payload = post(f"{api_base_url()}/{user_id}/media", access_token, {
                "image_url": public_asset_url("images", filename),
                "caption": parse_caption(caption_path),
            })
            creation_id = payload.get("id")
            if not creation_id:
                raise InstagramOnlyError("Instagram container response did not include an id.")
        ...
        save_queue(QUEUE_FILE, queue)
        print(f"Published Instagram-only post {post_id} (media id {media_id}).")
        return 0
    except (InstagramOnlyError, KeyError, OSError, json.JSONDecodeError) as exc:
        mark_failed(item, exc)
        save_queue(QUEUE_FILE, queue)
        print(f"Instagram-only post failed: {exc}")
        return 1
